...
# T2 RATE
def t2_rate(qubit, sparse: bool = True) -> torch.Tensor:
    ...
    def t1_rate(
        ...
        if noise_op.is_sparse:
            ...
            print(f"ground {ground.T.to(torch.complex128)}")

            rate = torch.sparse.mm(noise_op.to(torch.complex128), ground.T.to(torch.complex128))
            rate = torch.sparse.mm(excited.conj().to(torch.complex128), rate)
        ...
        if noise_op.is_sparse:
            ...
            rate = torch.abs(
                torch.sparse.mm(
                    ...
                    torch.sparse.mm(noise_op.to(torch.complex128), ground.T.to(torch.complex128)),
                )
                - torch.sparse.mm(
                    excited.conj().to(torch.complex128),
                    torch.sparse.mm(noise_op.to(torch.complex128), excited.T.to(torch.complex128)),
                )
            )
    ...
    t1_rate = effective_t1_rate(qubit, eigvecs=eigvecs, sparse=sparse, noise_channels=t1_noise_channels)
    tphi_rate = effective_tphi_rate(qubit, eigvecs=eigvecs, sparse=sparse, noise_channels=tphi_noise_channels)
